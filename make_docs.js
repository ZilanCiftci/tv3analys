...
const handledning = doc("Användarhandledning – tv3_analys", "Analys av TV-inspektioner av avloppsledningar från TV3-filer", [
  ...
  p("Skriptet `arcmap/skapa_ledningslager.py` skapar ett ledningslager av bedömningarna. Det körs i ArcMaps Python-fönster med kartan öppen:"),
  ...code(["execfile(r'H:\\PY\\Filmningar\\skapa_ledningslager.py')"]),
  spacer(),
  p("Skriptet läser `kartunderlag.json`, letar upp varje brunnspar i brunnslagret och klipper ut ledningen mellan brunnarna som ett eget objekt. Ledningar som passerar flera brunnar utan att vara uppdelade hanteras: varje vertexpunkt jämförs med närmaste brunn inom toleransen (standard 2 m), och `MAX_HOPP` styr hur många brunnar en sträcka får passera."),
  p("Lagernamn, sökvägar och tolerans anges i KONFIG överst i skriptet. Standardvärdena är `A Ledning`, `A Nedstign och övriga brunnar` och brunnsfältet `EntityID`."),
  ...
  p("När du fyllt i manuella bedömningar: kör skriptet igen med `BARA_UPPDATERA = True`. Då räknas `BEDOMNING`, `BED_TYP` och `STIL` om utan att geometrin byggs om. Vid en full omkörning läses befintliga manuella bedömningar in per brunnspar och återställs automatiskt."),
  h2("7.2 Symbologi"),
  p("Sätt symbologin en gång och spara som `.lyr`, så återanvänds den vid varje körning. I lagrets egenskaper: **Symbology > Categories > Unique values**, Value Field `STIL`, och **Add All Values**. Det ger tio kategorier:"),
  bullet("`A - Maskinell` … `E - Maskinell` – **streckad** linje i klassens färg."),
  bullet("`A - Manuell` … `E - Manuell` – **heldragen** linje i klassens färg."),
  p("Klassfärgerna är desamma som i Excel och diagrammen: rött A, orange B, gult C, grönt D, grått E. Spara sedan lagret som `.lyr` på sökvägen i `LYR_FIL`; nästa körning applicerar symbologin automatiskt."),
